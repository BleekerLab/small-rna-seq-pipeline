...
all.clusters.subset = dplyr::select(all.clusters,"sample","RPM","DicerCall")

# count number of clusters per dicer call
rpm_per_dicer_call = all.clusters.subset %>% 
  group_by(sample,DicerCall) %>%
  summarise(rpm_sum = sum(RPM))


# plot N clusters = f(DicerCall)
g <- ggplot(data = rpm_per_dicer_call,aes(x=DicerCall,y=rpm_sum,fill=DicerCall)) +
  geom_bar(stat="identity",color="black") +
  scale_fill_brewer(palette="Set3") +
  facet_wrap( ~ sample) +
  labs(x = "DicerCall",y="Reads Per Million")

# save the plots
ggsave(filename = file.path(output_directory_for_plots,"plots/abundance_of_clusters_per_dicer_call.png"),plot = g,width = 7,height = 5,dpi = 400,device = "png")
ggsave(filename = file.path(output_directory_for_plots,"plots/abundance_of_clusters_per_dicer_call.svg"),plot = g,width = 7,height = 5,device = "svg")
